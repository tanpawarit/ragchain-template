...
def load_vectorstore(
    ...
    """
    dvm = DataVersionManager(base_index_dir=os.path.dirname(cfg.faiss_index_path))
    faiss_index_path = dvm.get_data_version_path(data_version)
    ...
    embeddings = OpenAIEmbeddings(model=cfg.embedding_model_name, openai_api_key=cfg.openai_token)
    ...
    if mlflow_tracker:
        lineage_info = dvm.get_lineage(data_version)
        mlflow_tracker.log_params({
            "vectorstore_load_time_seconds": f"{load_time:.2f}",
            "vectorstore_data_version": lineage_info.get("data_version", "unknown"),
            "vectorstore_creation_time": lineage_info.get("creation_time", "unknown"),
        })
